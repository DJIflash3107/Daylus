<?php
$previmg = $data['file_location'];
$prevtitle = $data['title'];
$prevdescription = $data['description'];
$prevalbum = $data['album_id'];
?>
<div class="border-cust p-4 rounded-md shadow">
    <p class="text-base font-semibold mb-4">Edit Your Post</p>
    <form enctype="multipart/form-data" onsubmit="ValidateUpload(event)" class="flex flex-col gap-4" method="post">
        <label for="file-input">
            <div id="box" ondragenter="ImgEnter()" ondragleave="ImgLeave()" ondragover="AllowDrop(event)"
                ondrop="Drop(event)" class="box flex <?php if ($previmg !== null && $previmg !== "") {
                    echo "cust-max-h";
                } ?>">
                <div id="box-placeholder" class="m-auto -z-20 <?php if ($previmg !== null && $previmg !== "") {
                    echo "hidden";
                } ?>">
                    <img class="cutip" src="./assets/icons/album-black.svg">
                    <p>Browse or drag image</p>
                </div>
                <img id="prev-img" class="cust-koplo <?php if ($previmg == null || $previmg == "") {
                    echo "hidden";
                } ?>" src="<?php if ($previmg !== null && $previmg !== "") {
                     echo $previmg;
                 } ?>">
            </div>
        </label>
        ...
        <div class="input-cont p-2 border-cust rounded-md gap-2 polop">
            ...
            <input onblur="Validate('title')" id="title" name="title" class="input-polop" type="text"
                placeholder="Enter your picture title" <?php if ($prevtitle !== null && $prevtitle !== "") {
                    echo 'value="' . $prevtitle . '"';
                } ?>>
        ...
        <div class="input-cont p-2 border-cust rounded-md gap-2 polop">
            ...
            <textarea name="description" id="description" class="input-polop text-area" type="text"
                placeholder="Enter your picture description"><?php if ($prevdescription !== null && $prevdescription !== "") {
        ...
        <div class="input-cont p-2 border-cust rounded-md gap-2 polop">
            ...
            <select id="album" name="album" class="input-polop">
                ...
                while ($data = mysqli_fetch_array($res)) {
                    ?>
                    <option value="<?php echo $data['id']; ?>" <?php if ($prevalbum !== null && $prevalbum !== "" && $data['id'] == $prevalbum) {
                           echo "selected";
                       } ?>>
                        <?php $name = $data['name'];
        ...
        <button type="submit" class="btn-post" name="update">Update</button>
...
<script>
    ...
    function ValidateUpload(e) {
        ...
        if ((photo.value == "" && img.src == "") || title.value == "") {
            e.preventDefault();
        }
        if (photo.value == "" && img.src == "") {
            errorImage.classList.remove("hidden");
